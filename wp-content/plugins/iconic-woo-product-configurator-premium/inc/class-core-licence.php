...
if ( class_exists( 'Iconic_PC_Core_Licence' ) ) {
    return;
}
/**
 * Iconic_PC_Core_Licence.
 *
...
class Iconic_PC_Core_Licence
{
    ...
    {
        return $html . sprintf( '<a href="%s" class="button button-secondary">&larr; %s</a>', self::$args['urls']['settings'], __( 'Back to Settings', 'jckpc' ) );
    }
    ...
    {
        if ( !Iconic_PC_Core_Settings::is_settings_page( '-account' ) ) {
            return;
        }
        ?>
    ...
    {
        $links[] = sprintf( '<a href="%s" target="_blank">%s</a>', self::$args['urls']['product'] . '/changelog/?utm_source=jckpc&utm_medium=insideplugin', __( 'Changelog', 'jckpc' ) );
        return $links;
    }
    ...
    {
        return sprintf( '<a href="%s" class="button button-secondary">%s</a>', self::$args['urls']['account'], __( 'Manage Licence &amp; Billing', 'jckpc' ) );
    }
