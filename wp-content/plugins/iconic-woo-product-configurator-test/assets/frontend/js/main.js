...
(function( $, document ) {
		...
		function clearLayer( layer_id ) {
			...
			var $img = select_value.length <= 0 ? getDefaultImage( layer_id ) : '';
		...
		function load_image_layer( selectedVal, selectedAtt ) {
			...
			var ajaxargs = {
				...
				'request_id': load_id,
				'selected_attributes': $variationsForm.serializeArray()
			};
			...
			$.ajax( {
				...
				if ( response.success ) { console.log( response.data );
					if ( ! $.isEmptyObject( response.data.images ) ) {
						$.each( response.data.images, function( attribute, image_data ) {
							var layer_id = getLayerId( attribute );

							// loaded_images[ layer_id ][ selectedVal ] = data.image;
							update_image_layer( layer_id, image_data.html, response.data.request_id );
						} );
					}
				}
		...
		function increment_load_id() {
			...
			$selects.each( function( index, select ) {
				...
				var layer_id = getLayerId( select_data.attribute );
		...
		function update_large_image() {
			...
			$( '.iconic-pc-image--background img' ).attr( 'data-large_image', url );
