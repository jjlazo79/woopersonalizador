...
* Add template to
*/
// Customizer enqueue_script only if simple product need
function handbag_customizer(){
    wp_enqueue_script(
        'handbag-customizer',
        get_stylesheet_directory_uri() . '/lib/woocommerce/js/handbag-customizer.js',
        array( 'jquery' ),
        CHILD_THEME_VERSION,
        true
    );
...
function custom_single_product_template_include( $template ) {
   if ( is_singular('product') && (has_term( 'personalizador', 'product_cat')) ) {
       $template = get_stylesheet_directory() . '/woocommerce/single-product-personalizador.php';
        add_action( 'wp_enqueue_scripts', 'handbag_customizer' );
